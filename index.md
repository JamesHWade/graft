...
accepted change came from, and keeps earlier versions available — then
hands that accepted knowledge to an agent as bounded, read-only tools.
...
explainable as they change. They are also what makes accepted knowledge
...
compiles the resolved JSON using R alone, then creates the store — no
...
Candidate records are ordinary data frames. This batch has a valid
person and organization, but its employment row points to an
...
Planning writes nothing, so a rejected batch can be corrected and
reviewed again.
[`graft_commit()`](https://jameshwade.github.io/graft/reference/graft_commit.md)
rechecks the reviewed plan and accepts all three records in one
...
A later review changes Lois’s role. The update is another reviewable
plan, not an overwrite:
...
An agent is only as trustworthy as the knowledge it reads. graft gives a
model records that were validated on the way in, attributed to a
producer, and pinned so they cannot move mid-session.
...
The tools search, retrieve, run bounded advanced operations, and read
revision history. They accept no SQL, connection, path, or write
argument, and every result reports the limit it applied, whether it was
truncated, and the digest of the contract it came from. Fields the
contract marks restricted never reach them.

Writes stay on the reviewed path. An agent that proposes records becomes
a named producer whose plan is validated and inspected like any other,
and a file-editing agent works through a readable Open Knowledge Format
tree whose edits remain proposals until they are reviewed. [Work with
...
- [Get
  ...
  works through this example with its results, then hands the store to
  an agent.
- [Use a data-dict
  contract](https://jameshwade.github.io/graft/articles/data-dict-schema.md)
  covers table-first authoring, compilation, and what the table profile
  enforces.
- [Review knowledge
...
- [Work with
  agents](https://jameshwade.github.io/graft/articles/agents.md) covers
  bounded tools, pinned snapshots, agent-authored proposals, and
  file-editing agents.
