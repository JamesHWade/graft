...
manifest_class_hierarchy_problem <- function(classes, data_dict) {
  ...
  for (class_name in names(classes)) {
    if (identical(class_name, graft_definition_class_name)) {
      next
    }
...
manifest_semantic_contract_problem <- function(manifest, data_dict) {
  ...
  user_classes <- manifest$classes[
    setdiff(class_names, graft_definition_class_name)
  ]
  ancestors <- unlist(
  ...
  for (class_name in class_names) {
    ...
    if (identical(class_name, graft_definition_class_name)) {
      contract_matches <- identical(
        ...
        canonical_json(graft_definition_class_contract())
      )
      if (!contract_matches) {
        return(manifest_contract_problem(
          "The GraftDefinition system class does not match its fixed contract.",
          paste0("classes.", class_name),
...
validate_data_dict_projection_contract <- function(manifest, path) {
  ...
  observed_projection$classes[[graft_definition_class_name]] <- NULL
  if (
