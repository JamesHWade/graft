test_that("closed backups verify and restore a complete local store", {
  ...
  selection <- artifact_select(source, list(report))
  accepted <- artifact_decide(
    source,
    "report-review",
    "review-1",
    expected = NULL,
  ...
  expect_identical(
    graft_verify_backup(paste0(backup_path, "/."), receipt),
    receipt
  )
  expect_identical(graft_manifest(source), source_manifest)

  target <- graft_store(withr::local_tempdir(), create = TRUE)
  restored <- graft_restore(backup_path, target, receipt)

  expect_identical(restored, source_manifest)
  expect_identical(graft_manifest(target), source_manifest)
  expect_identical(
    artifact_read(target, report)$bytes,
    charToRaw("report bytes")
  )
  expect_identical(
    artifact_read_decision(target, "report-review"),
    accepted
  )
})
...
test_that("backup verification requires the external receipt and canonical image", {
  ...
  backup_path <- file.path(withr::local_tempdir(), "closed-backup")
  receipt <- graft_backup(source, backup_path, "reader-a", "gen-1")
  ...
  content_path <- file.path(
    ...
    "objects",
    "content",
    artifact_read(source, ref)$metadata$payload
  )
...
test_that("backup paths cannot overwrite or overlap a source store", {
  ...
  dir.create(existing)
  expect_error(
    graft_backup(source, existing, "reader", "generation"),
    class = "graft_artifact_error"
  )
  ...
  expect_error(
    graft_backup(source, inside, "reader", "generation"),
    class = "graft_artifact_error"
  )
...
test_that("bundle root aliases and special files are rejected before reads", {
  ...
  source <- graft_store(withr::local_tempdir(), create = TRUE)
  backup_path <- file.path(withr::local_tempdir(), "closed-backup")
  receipt <- graft_backup(source, backup_path, "reader", "generation")

  alias <- file.path(withr::local_tempdir(), "alias")
  expect_identical(file.symlink(backup_path, alias), TRUE)
  expect_error(
    graft_verify_backup(paste0(alias, "/."), receipt),
    class = "graft_artifact_error"
  )
  ...
  result <- callr::r(
    function(checkout, path, expected) {
      ...
      tryCatch(
        graft::graft_verify_backup(path, expected),
...
test_that("backup and verification bounds are caller supplied", {
  ...
  receipt <- graft_backup(source, backup_path, "reader", "generation")
  expect_error(
    graft_verify_backup(backup_path, receipt, max_bytes = 1),
    class = "graft_artifact_error"
  )
})
...
test_that("backup rechecks a source changed during copying", {
  ...
  expect_error(
    graft_backup(source, backup_path, "reader", "generation"),
    class = "graft_artifact_error"
  )
...
test_that("restore rechecks a target changed during copying", {
  ...
  expect_identical(changed, TRUE)
  expect_gt(length(graft_manifest(target)$objects), 0L)
  expect_identical(graft_verify_backup(backup_path, receipt), receipt)
})
...
test_that("existing parent aliases resolve before source-overlap checks", {
  ...
  source <- graft_store(file.path(parent, "source"), create = TRUE)
  before <- graft_manifest(source)
  path <- file.path(alias, "backup")
  receipt <- graft_backup(source, path, "reader", "generation")
  expect_identical(graft_verify_backup(path, receipt), receipt)
  expect_error(
    graft_backup(
      source,
