...
`display: restricted` is compiled as sensitive: its values are dropped
from retrieved records, excluded from searchable fields, and never reach
a tool built with
[`graft_tools()`](https://jameshwade.github.io/graft/reference/graft_tools.md).
Primary IDs are public operational keys and cannot be restricted. This
is the contract-level control over what an agent can see; see [Work with
