...
contract 0.8.0. Host decision recording and guarded consultation ship
with consumer contract 1.0.0. Tempest now publishes research evidence
and synthesis through this artifact contract and checks fresh admission
for new runs and resume; see the [artifact
...
Consumer contract 1.0.0 is a preproduction hard cut, not a production
package release. The legacy data-dict compiler fingerprints the full
Graft namespace, so adding the decision functions changes its identity
even though compilation rules are unchanged. Recompile data-dict
manifests from retained resolved JSON or YAML. Graph stores tied to an
earlier compiled manifest are not supported for seamless reopening;
recreate disposable stores from retained inputs where needed. The opaque
