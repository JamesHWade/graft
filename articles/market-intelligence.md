...
The five views expose separate parts of the contract. The primary path
stays deliberately quiet: read the daily brief, inspect what accepted
memory changed, and open the decision room only when a proposal needs
judgment.

- **Briefing** separates what changed, why it matters, the effect of
  accepted memory, and one proposed next step. Materiality, confidence,
  and evidence counts remain visible without competing with the
  narrative.
- **Portfolio map** shows enterprise overlap, business-specific
  competitors, and downstream lenses.
- **Review** presents observed evidence, analyst interpretation, and
  proposed action as distinct layers. It states the Graft consequence of
  approval or rejection before the user decides.
...
Approved radar knowledge can be published as a source-linked OKF bundle
for other agents and documentation tools without granting them access to
the DuckDB store:
...
kg_export_okf(
  worker$graft,
  "knowledge/materials-market-radar",
  classes = c(
    "Business",
    "Competitor",
    "DownstreamMarket",
...
The export contains accepted revisions only. A proposed assessment
remains absent until the operator approves the Tempest change set and
the host commits it to Graft. Continue with [Share accepted knowledge
with
OKF](https://jameshwade.github.io/graft/articles/open-knowledge-format.md)
for the exact mapping and Tempest handoff.
