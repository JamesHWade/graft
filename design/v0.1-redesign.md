...
objects. LinkML remains the domain type system, data frames remain the bulk
record boundary, and DuckDB remains the only storage backend.
...
```mermaid
flowchart LR
    L["LinkML contract"] --> S["Validated workflow records"]
...
Graft owns the semantic contract, stable identity, validation, accepted record
history, provenance, and bounded retrieval. Host applications own workflow
...
active LinkML contract. Multivalued relations, graph edges, search rows, and
other performance indexes will also be generated projections. They may be SQL
views, transactionally maintained tables, or versioned caches, provided that:
...
- Dynamic S7 classes for each LinkML domain class.
...
- [ ] Capture the current correctness tests as behavior-level requirements,
  separating durable semantics from legacy representation details.
- [ ] Add a reproducible ingestion benchmark for 100 and 1,000 records.

### Phase 1: Separate the product from its demonstrations

- [ ] Move the three installable applications and their host-specific tests to
  a companion home.
- [ ] Remove Tempest-specific exports, adapters, remotes, and documentation from
  core Graft.
- [ ] Retain one provider-free core walkthrough of the complete knowledge loop.

### Phase 2: Establish the semantic model and facade

- [ ] Add the targeted S7 classes, property types, validators, and tests.
- [ ] Move S7 to `Imports` and raise the minimum R version.
- [ ] Introduce the `graft_*` facade and route it through the new objects.
- [ ] Keep any temporary `kg_*` bridge internal and mark it for deletion.

### Phase 3: Build the revision-first store

- [ ] Define the new store-format metadata and append-only durable tables.
- [ ] Implement deterministic reconstruction and verification of record heads.
- [ ] Generate class-shaped current views from active revisions.
- [ ] Generate and rebuild relation, graph, and search projections.
- [ ] Prove that dropping derived projections does not lose accepted knowledge.

### Phase 4: Implement planning

- [ ] Normalize all input into a common staging relation.
- [ ] Resolve identity and validate references in bulk.
- [ ] Compare staged content with current revisions in bulk.
- [ ] Produce deterministic `GraftCommitPlan` objects and aggregated issues.
- [ ] Test read-only behavior, determinism, tamper evidence, and stale heads.

### Phase 5: Implement committing

- [ ] Recheck every plan precondition before mutation.
- [ ] Append batches, identity decisions, revisions, heads, and observations in
  one transaction using bulk operations.
- [ ] Implement replay-safe result recovery.
- [ ] Verify atomic rollback for failures at each write stage.
- [ ] Meet the database-statement and performance acceptance criteria.

### Phase 6: Unify retrieval, history, and OKF

- [ ] Port bounded retrieval and history to the ledger and derived views.
- [ ] Route OKF edits through `graft_plan()` and `graft_commit()`.
- [ ] Generate agent tools from the shared retrieval contract.
- [ ] Update integrity diagnostics for authoritative and derived state.

### Phase 7: Remove the legacy architecture

- [ ] Delete manifest-defined current tables and generated relation writes.
- [ ] Delete physical migration planning and application.
- [ ] Remove duplicate validation, write, and retrieval paths.
- [ ] Remove all remaining public compatibility shims.

### Phase 8: Document and qualify v0.1

- [ ] Rewrite the README and getting-started material around contract, plan,
  commit, query, history, and review.
- [ ] Rebuild fixtures under the new store format.
- [ ] Run formatting, focused tests, the full test suite, package checks, and
  documentation checks.
- [ ] Conduct an adversarial design, code, performance, and test review before
  merging.

## Stable code references

The current implementation areas to replace or reuse are:

| Concern | Current location | Key symbols |
|---|---|---|
| Ingestion transaction | `R/ingest.R` | `kg_ingest()`, `write_staged_records()` |
| Revision canonicalization | `R/revisions.R` | `logical_record_payload()`, `write_staged_revisions()` |
| Validation and staging | `R/validation.R` | `kg_validate_data()`, `prepare_ingest_records()` |
| Identity resolution | `R/identity.R` | `resolve_record_identity()` |
| Value objects | `R/core-objects.R` | `new_kg_schema()`, `new_kg_store()`, `new_kg_batch()` |
| OKF review | `R/okf-import.R` | `kg_plan_okf_import()`, `kg_apply_okf_import()` |
| Physical migrations | `R/migration.R` | `kg_plan_migration()`, `kg_apply_migration()` |
| Historical decision | `design/knowledge-change-control.md` | Typed projection and migration decisions |
