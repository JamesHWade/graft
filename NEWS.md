...
* `graft_blob()` and `graft_blob_read()` keep content-addressed bytes beside the store for contracts that declare a `graft_blob` table; planning fails when a blob's bytes are missing or differ from its record, committed blobs are immutable, and reads verify size and digest.
